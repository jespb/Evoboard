...
float mutationRateAdd = 0.3;
float mutationRateRemove = 0.1;
float mutationRateModify = 0.1;

int resolution = 128;
String pathTargetImage = "data_rmse/Z.png";
...
void setup() {
  ...
  pop = new Population();
  cells = calculateGrid(populationSize, 0, 0, width, height - 30, 30, 10, 30, true);
...
void draw() {
  ...
  for (int i = 0; i < pop.getSize(); i++) {
    ...
    if (phenotypeMode) {
      ...
      noFill();
      strokeWeight(max(cellDim * 0.02, 1));
      stroke(255);
      pop.getIndiv(i).renderOutline(getGraphics(), cells[row][col].x, cells[row][col].y, cellDim);
    }
    if (showFitness) {
      fill(phenotypeMode ? 80 : 200);
      textSize(constrain(cellDim * 0.2, 11, 14));
  ...
  float sumNumPoints = 0;
  for (int i = 0; i < pop.getSize(); i++) {
    sumNumPoints += pop.getIndiv(i).points.size();
  }
  int avgNumPoints = round(sumNumPoints / (float) pop.getSize());
  
  fill(0, 0, 255);
  textSize(20);
  textAlign(LEFT);
  translate(30, height - 30);
  text("Generations: " + pop.getGenerations() + "  |  Avg num points: " + avgNumPoints, 0, 0);
